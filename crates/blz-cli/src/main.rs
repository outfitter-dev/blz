...
use blz_core::{
    ...
    Registry, ResourceMonitor, SearchIndex, Source, Storage,
};
...
async fn search(
    ...
    if let Some(percentile) = top_percentile {
        let percentile_count =
            (all_hits.len() as f32 * (percentile as f32 / 100.0)).ceil() as usize;
    ...
    // Apply pagination
    let actual_limit = if limit >= 10000 {
        all_hits.len()
    } else {
        limit
    };
    ...
    if start_idx >= all_hits.len() {
        println!(
            ...
            page,
            (total_results + actual_limit - 1) / actual_limit
        );
        return Ok(());
    }
...
mod tests {
    ...
    fn test_line_range_parsing_invalid_format() {
        let invalid_formats = [
            "abc", "10-", "-10", "10:", ":10", "10+", "+10", "10--20", "10::20",
        ];
    ...
    mod test_helpers {
        ...
        pub fn create_test_storage_with_sources(sources: &[&str]) -> (Storage, TempDir) {
            ...
            for &source in sources {
                storage
                    .ensure_tool_dir(source)
                    .expect("Failed to create tool dir");
            }
    ...
    fn test_reserved_keywords_completeness() {
        ...
        for &cmd in &expected_commands {
            assert!(
                RESERVED_KEYWORDS.contains(&cmd),
                "Command '{}' should be in reserved keywords",
                cmd
            );
        }
    }
    ...
    fn test_invalid_percentile_values() {
        ...
        for &percentile in &invalid_percentiles {
            ...
            assert!(
                ...
                "Percentile {} should be considered invalid",
                percentile
            );
        }
    ...
    fn test_line_range_error_messages() {
        ...
        assert!(
            error.contains("1"),
            "Error should mention line numbers start at 1"
        );
        ...
        let error = result.unwrap_err().to_string();
        assert!(
            error.contains("10") || error.contains("5"),
    ...
    fn test_exact_file_detection() {
        ...
        for url in &non_exact_urls {
            ...
            assert!(
                !is_exact,
                "URL {} should NOT be detected as exact file",
                url
            );
        }
    }
