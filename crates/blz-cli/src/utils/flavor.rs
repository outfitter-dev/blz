...
use blz_core::{Fetcher, FileInfo, LineIndex, LlmsJson, ParseMeta, ParseResult, Source, Storage};
use chrono::Utc;
...
use crate::utils::store::{self, SourceOverrides};

pub const BASE_FLAVOR: &str = "llms";
pub const FULL_FLAVOR: &str = "llms-full";
...

    {
        ...
        if flavor.is_some() {
            record
                ...
                .or_insert_with(SourceOverrides::default)
...
fn normalize_flavor(raw: &str) -> Option<String> {
...
mod tests {
    ...
    fn with_temp_env<F: FnOnce(Storage, &std::path::Path) -> Result<()>>(f: F) -> Result<()> {
        ...
        let result = f(storage, config_dir.path());

        unsafe {
            std::env::remove_var("BLZ_CONFIG_DIR");
            std::env::remove_var("BLZ_GLOBAL_CONFIG_DIR");
        }
        ...
        with_temp_env(|storage, _config_dir| {
            ...
            assert!(
                !store
                    .scopes
                    .get(&key)
                    .map_or(false, |record| record.sources.contains_key("vue"))
            );
