...
mod tests {
    ...
    fn test_registry_entry_with_aliases() {
        let entry = RegistryEntry::new("Node.js", "node", "JavaScript runtime", "https://nodejs.org/llms.txt")
            .with_aliases(vec!["node", "nodejs", "js"]);
    ...
    fn test_all_registry_entries_have_valid_urls() {
        ...
        for entry in registry.all_entries() {
            ...
            assert!(entry.llms_url.starts_with("http://") || entry.llms_url.starts_with("https://"));
    ...
    fn test_registry_entries_have_unique_slugs() {
        ...
        for entry in entries {
            assert!(slugs.insert(&entry.slug), "Duplicate slug found: {}", entry.slug);
        }
    }
}