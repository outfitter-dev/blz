use blz_core::{MarkdownParser, SearchIndex, HeadingBlock, PerformanceMetrics};
...
fn setup_index_with_blocks(blocks: &[HeadingBlock]) -> (TempDir, SearchIndex) {
    ...
    index.index_blocks("bench", "test.md", blocks)
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    ...
                    index.search(query, Some("bench"), 10)
...
fn bench_query_complexity(c: &mut Criterion) {
    ...
    for (name, query) in &queries {
        group.bench_with_input(
            ...
            |b, query| {
                b.iter(|| {
                    index.search(black_box(query), Some("bench"), 20)
                        .expect("Search failed")
                });
            },
        );
...
fn bench_result_limits(c: &mut Criterion) {
    ...
    for &limit in &limits {
        group.bench_with_input(
            ...
            |b, &limit| {
                b.iter(|| {
                    index.search(black_box("performance"), Some("bench"), limit)
                        .expect("Search failed")
                });
            },
        );
...
fn bench_content_size_impact(c: &mut Criterion) {
    ...
    for &size in &content_sizes {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    index.search(black_box("authentication"), Some("bench"), 10)
                        .expect("Search failed")
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, blocks| {
                b.iter_with_setup(
                    ...
                    |(temp_dir, mut index)| {
                        index.index_blocks("bench", "test.md", black_box(blocks))
                            .expect("Failed to index blocks");
                        // Keep temp_dir alive
                        drop(temp_dir);
                    }
...
fn bench_realistic_workload(c: &mut Criterion) {
    ...
    for (name, block_count, content_size) in &scenarios {
        ...
        let total_mb = (blocks.iter().map(|b| b.content.len()).sum::<usize>() as f64) / (1024.0 * 1024.0);
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    ...
                    let queries = ["React", "hooks useState", "performance", "TypeScript interface"];
                    for query in &queries {
                        let _results = index.search(black_box(query), Some("bench"), 10)
                            .expect("Search failed");
                    }
                });
            },
...
fn bench_performance_targets(c: &mut Criterion) {
    ...
    group.bench_function("target_search_10ms", |b| {
        b.iter(|| {
            let result = index.search(black_box("React hooks"), Some("bench"), 10)
                .expect("Search failed");
    ...
    group.bench_function("target_multi_search_50ms", |b| {
        b.iter(|| {
            let queries = ["React", "hooks", "TypeScript", "performance", "authentication"];
            for query in &queries {
                let _result = index.search(black_box(query), Some("bench"), 5)
                    .expect("Search failed");
            }
        });
    });
