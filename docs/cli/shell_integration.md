...
- Positional alias completion for `blz get`, `blz refresh` (and deprecated `blz update`), `blz remove`, `blz anchors`, and `blz anchor list|get`
...
- `--source`/`-s` dynamic values for `blz search` (also supports `--alias` for backward compatibility)
- Positional alias completion for `blz get`, `blz refresh` (deprecated alias: `blz update`), `blz remove`, `blz diff`, `blz anchors`
